...
namespace MYGIS
{
    ...
    public class GISExtent //这是一个重要的类 空间范围
    {
        ...
        {
            upright = new GISVertex(extent.upright);//深度复制左下、右上的点
    ...
    public class GISView
    {
        ...
        public void Update(GISExtent _extent, Rectangle _rectangle)
        {
            ...
            ScaleX = Math.Max(ScaleX, ScaleY);
            ScaleY = ScaleX;
            MapW = MapWindowsSize.Width * ScaleX;
            MapH = MapWindowsSize.Height * ScaleY;
            GISVertex center = CurrentMapExtent.GetCenter();
            MapMinX = center.x - MapW / 2;
            MapMinY = center.y - MapH / 2;
            //CurrentMapExtent = new GISExtent(new GISVertex(MapMinX, MapMinY),
            //    new GISVertex(MapMinX + MapW, MapMinY + MapH));
        }
        ...
        internal void UpdateWindowSize(Rectangle client)
        {
            Update(CurrentMapExtent, client);
        }

        internal void UpdateRectangle(Rectangle clientRectangle)
        {
            MapWindowsSize = clientRectangle;
            Update(CurrentMapExtent, MapWindowsSize);
        }

        internal GISExtent getRealExtent()
        {
            return new GISExtent(new GISVertex(MapMinX, MapMinY),
                new GISVertex(MapMinX + MapW, MapMinY + MapH));
        }

        public GISExtent Rect2Extent(int x, int mouseStartX, int y, int mouseStartY)
        {
            GISVertex v1 = ToMapVertex(new Point(x, y));
            GISVertex v2 = ToMapVertex(new Point(mouseStartX, mouseStartY));
    ...
    public class GISLayer
    {
        ...
        public string Path = "";
        //public GISThematic Thematic;//记录唯一值专题地图的制图样式
        ...
        //    {
        //        //如果空间对象范围与地图范围不相交则不绘制 这样可以提高速度
        //        if (Features[i].spatialpart.extent.IntersectOrNot(view.CurrentMapExtent))
        ...
        public void MakeUniqueValueMap(int FieldIndex)
        {
            ...
            {
                //if (Thematics.ContainsKey(o))
                //{
                //    continue;

                //}
                //用insidecolor来区分不同的独立值空间对象
    ...
    public class GISSelect
    {
        ...
        public SelectResult Select(GISExtent extent, List<GISFeature> Features)
        {
            ...
            for (int i = 0; i < Features.Count; i++)
            {
                if (extent.inclue(Features[i].spatialpart.extent))
    ...
    public class GISConst
    {
        ...
        //专题地图默认点、线、面的颜色
        public static Color defaultPointColor = Color.Pink;
        public static Color defaultLineColor = Color.CadetBlue;
        public static Color defaultPolygonEdgeColor = Color.Black;
        public static Color defaultPolygonFillColor = Color.Cyan;
    }
    ...
    public class GISDocument
    {
        ...
        public GISLayer Addlayer(string fileName)
        {
            GISLayer layer = null;
            string filetype = System.IO.Path.GetExtension(fileName).ToLower();
            //根据不同的文件类型调用不同的文件打开函数
            if (filetype == "." + GISConst.SHPFILE)
            {
                layer = GISShapefile.ReadShapefile(fileName);
            }
            else if (filetype == "." + GISConst.MYFILE)
            {
                layer = GISMyFile.ReadFile(fileName);
            }
            layer.Path = fileName;
            getUniqueName(layer);//确保每个图层名独一无二
            layers.Add(layer);
            UpdateExtent();
            return layer;
        }
        ...
        internal GISLayer getLayer(string layername)
        {
        ...
        public void Write(string fileName)
        {
            ...
            for(int i=0;i<layers.Count;i++)
            {
                GISTools.WriteString(layers[i].Path, bw);//写路径
                bw.Write(layers[i].DrawAttributeOrNot);//写属性显示情况
                bw.Write(layers[i].LabelIndex);//写标签索引号
                bw.Write(layers[i].Selectable);//写选中情况
                bw.Write(layers[i].Visible);//写可视情况
            }
            bw.Close();
            fsr.Close();
        }
    ...
    {
        public Color OutsideColor;//点对象边界颜色
        public int Size;//点半径 线面对象边界宽度
        public Color InsideColor;//点、面填充色 线的颜色
        ...
        public GISThematic(SHAPETYPE shapetype)
        {
            if (shapetype == SHAPETYPE.point)
            {
                Update(GISConst.defaultPointColor,
                    GISConst.PointSize, GISConst.defaultPointColor);
            }
            else if(shapetype == SHAPETYPE.line)
            {
                Update(GISConst.defaultLineColor,
                    GISConst.LineWidth, GISConst.defaultLineColor);
            }
            else if (shapetype == SHAPETYPE.polygon)
            {
                Update(GISConst.defaultPolygonEdgeColor,
                    GISConst.PolygonBoundaryWidth, GISConst.defaultPolygonFillColor);
            }
        }
