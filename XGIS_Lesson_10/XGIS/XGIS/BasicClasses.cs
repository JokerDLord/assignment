...
namespace XGIS
{
    ...
    public class XLayer
    {
        ...
        public void SelectByInterExtent(XExtent extent)
        {
            foreach (XFeature feature in Features)
            {
                feature.Selected = extent.IntersectWith(feature.Spatial.Extent);
            }
        }
        ...
        public void Draw(Graphics _Graphics, XView _View)
        {
            for (int i = 0; i < Features.Count; i++)
            {
                if (Features[i].Spatial.Extent.IntersectWith(_View.CurrentMapExtent))
                    Features[i].Draw(_Graphics, _View, DrawAttributeOrNot, LabelIndex, Selected);
            }
        }
    ...
    public class XExtent
    {
        ...
        {
            return new XVertex((UpRight.X + BottomLeft.X) / 2,
                (UpRight.Y + BottomLeft.Y) / 2);
        }
        ...
        {
            return (extent.GetMinX() > GetMinX() && extent.GetMaxX() < GetMaxX()
                && extent.GetMinY() > GetMinY() && extent.GetMaxY() > GetMaxY());
        }
    }
    ...
    public abstract class XSpatial
    {
        ...
        public abstract void Draw(Graphics _Graphics, XView _View, bool Selected);
    ...
    public class XFeature
    {
        ...
        public void Draw(Graphics _Graphics, XView _View, bool _DrawAttributeOrNot, int _Index,bool Selected)
        {
            Spatial.Draw(_Graphics, _View, Selected);
    ...
    public class XPointSpatial:XSpatial
    {
        ...
        public override void Draw(Graphics _Graphics, XView _View, bool Selected)
        {
